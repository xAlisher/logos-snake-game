...
## Finding #23: logoscore Loads Module But Universal Interface Crashes at Runtime
**Severity:** Critical
**Step:** Testing with logoscore
**Details:** logoscore successfully discovers, validates, and launches the high_score module in a separate `logos_host` process. But the process exits with code 1 immediately — the `LogosProviderBase` class (generated by code generator) is not compatible with the `PluginInterface` pattern used by this runtime build.
**Key logs:**
- `Found plugin: high_score_plugin.so` ✓
- `Plugin "high_score" is now running in separate process` ✓
- `Plugin process finished: "high_score" Exit code: 1` ✗
- `Timeout waiting for object replica to be ready` ✗
**Root cause:** Universal interface generates `LogosProviderBase`-based plugin. Our runtime expects `PluginInterface`. Incompatible.

## Finding #24: Platform Variant Naming Mismatch
**Severity:** Medium
**Step:** Testing with logoscore
**Details:** `nix build .#lgx` produces variant name `linux-amd64-dev` (with `-dev` suffix). logoscore/runtime expects `linux-amd64` or `linux-x86_64`. Module not discovered until manifest manually fixed.
**Recommendation:** Builder should produce `linux-amd64` for release and `linux-amd64-dev` only for explicit dev builds.

## Finding #25: Our Working AppImage Source — Local Nix Build
**Severity:** Info
**Step:** Investigation
**Details:** Our working `logos-app.AppImage` was built from a local Nix build at `/nix/store/...-logos-app-1.0.0`. It's called `LogosApp` (not `LogosBasecamp`), uses `LogosApp/` data dir for modules and `LogosBasecamp/` for plugins, has bundled `logoscore` and `logos_host`, and supports file-drop module discovery. The GitHub `logos-co/logos-basecamp` releases are a completely different build that does NOT support file-drop.
